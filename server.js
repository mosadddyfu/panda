...
// MarketApp API key fallback (for testing without env vars)
// NOTE: For production, prefer setting process.env.MARKETAPP_AUTH and rotate this key if committed.
const MARKETAPP_KEY_FALLBACK = 'AGWJIYHB6K5YNKYAAAAHFPEBZ4DCVRMJ6H6PQJBOTLEIALPAOWMC5QJOZLCTWP37TY5MPHA';
...
// Lookup Telegram recipient info (photo, display name) via MarketApp API
// Frontend calls this to avoid exposing the API key.
app.post('/recipient', async (req, res) => {
  try {
    ...
    const apiKey = process.env.MARKETAPP_AUTH || MARKETAPP_KEY_FALLBACK;

    const authHeader = (/^(Bearer|Token)\s+/i).test(apiKey) ? apiKey : `Bearer ${apiKey}`;
    const uname = username.startsWith('@') ? username : `@${username}`;

    const { data } = await axios.post(
      'https://api.marketapp.ws/v1/fragment/stars/recipient/',
      { username: uname },
      { headers: { Authorization: authHeader, 'Content-Type': 'application/json', Accept: 'application/json' } }
    );

    // Expecting { photo: string, name: string }
    return res.json({ photo: data.photo, name: data.name });
  } catch (err) {
    const status = err.response?.status || 500;
    if (status === 401) {
      return res.status(401).json({ error: 'Unauthorized to MarketApp. Check MARKETAPP_AUTH.' });
    }
    if (status === 404) {
      return res.status(404).json({ error: 'User not found' });
    }
    return res.status(status).json({ error: err.response?.data || 'upstream error' });
  }
});
